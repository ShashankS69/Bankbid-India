...
function ExpandButton({ onClick, compact = false }) {
  return (
    <button
      onClick={(e) => {
        e.preventDefault();
      ...
      } font-mono text-gold uppercase tracking-widest self-start hover:text-gold/80 transition-colors mt-auto pt-1`}
    >
      Show more ▾
    </button>
  );
...
// Full-detail popup. Rendered via portal so it sits above everything
// (including the grid's overflow/stacking context) and dims the whole page.
function PropertyModal({
  ...
  return (
    ...
    >
      <article
        className="lot-ticket rounded-sm p-6 pt-7 flex flex-col gap-4 w-full max-w-md max-h-[85vh] overflow-y-auto relative"
      ...
      >
        ...
        <button
          onClick={onClose}
          aria-label="Close"
          className="absolute top-4 right-4 text-slate hover:text-gold text-xl leading-none"
        >
          ✕
        </button>

        <header className="flex items-start justify-between gap-3 pr-6">
          <div>
          ...
          <div className="shrink-0 flex flex-col items-end gap-1.5">
            <FavoriteButton active={favorited} onClick={onToggleFavorite} />
            {isExpired ? (
        ...
        {source_url && (
          <a
            ...
            className="text-[10px] font-mono text-gold uppercase tracking-wide underline underline-offset-2 hover:text-gold/80"
          >
            {sourceLinkLabel}
          </a>
        )}
...
export default function PropertyCard({ listing, compact = false }) {
  ...
  // Everything the modal shows on top of what the card always shows.
  const hasExtra = Boolean(
    area_sqft_estimated || residex_comparison || roi_estimate || sourceLabel || source_url
  );
  ...
  const modal =
    mounted && showModal
      ? createPortal(<PropertyModal {...modalProps} />, document.body)
      : null;
  ...
  if (compact) {
    return (
      <>
        <article
          onClick={() => hasExtra && setShowModal(true)}
          className={`lot-ticket rounded-sm p-3.5 pt-4 flex flex-col gap-2.5 transition-transform hover:-translate-y-0.5 min-w-0 h-[280px] ${
        ...
        >
          ...
          {hasExtra && <ExpandButton compact onClick={() => setShowModal(true)} />}
        </article>
        {modal}
      </>
    );
  ...
  return (
    <>
      <article
        onClick={() => hasExtra && setShowModal(true)}
        className={`lot-ticket rounded-sm p-5 pt-6 flex flex-col gap-4 transition-transform hover:-translate-y-0.5 h-[380px] ${
          hasExtra ? "cursor-pointer" : ""
        }`}
      >
        <span className="lot-notch-l" aria-hidden="true" />
        ...
        <header className="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 className="font-display text-lg text-ink mt-1 leading-snug line-clamp-2">
              {typeLabel} · {place || "Location undisclosed"}
            </h3>
          </div>
          <div className="shrink-0 flex flex-col items-end gap-1.5">
        ...
        {hasExtra && <ExpandButton onClick={() => setShowModal(true)} />}
      </article>
      {modal}
    </>
  );
