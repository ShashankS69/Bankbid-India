...
export default function CalendarView({ lots, loading, error, onSelectDate }) {
    ...
    <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
      ...
      <div style={{ flex: "1 1 480px", minWidth: 320 }}>
        ...
        <div style={gridStyle}>
          {monthGrid.map((d, i) => {
            if (!d) {
              return <div key={i} style={emptyCell} />;
            }
      ...
      {/* Date to be announced panel */}
      <div style={{ flex: "1 1 320px", minWidth: 280 }}>
        <button
          onClick={() => setShowTBA((s) => !s)}
          style={tbaToggle}
        >
          {showTBA ? "▾" : "▸"} Date to be announced (
          {undated.length.toLocaleString("en-IN")})
        </button>
        ...
        {showTBA && (
          <div style={{ ...panelList, marginTop: 8 }}>
            ...
            >
              Mostly IBAPI listings — this source rarely publishes an auction
              date upfront.
            </div>

            {undated.slice(0, 100).map((lot) => (
              <LotRow
                key={`${lot.source}-${lot.property_id}`}
                lot={lot}
              />
            ))}
            ...
            {undated.length > 100 && (
              ...
              >
                +{(undated.length - 100).toLocaleString("en-IN")} more — filter
                by bank or use search to narrow down.
              </div>
            )}
          </div>
        )}
...
function LotRow({ lot }) {
  return (
    <div style={lotRow}>
      ...
      >
        {lot.property_type} · {lot.location}
        {lot.state ? `, ${lot.state}` : ""}
      </div>
...
const navButton = {
  ...
  border: "1px solid rgba(242, 140, 38, 0.28)",
  color: "#F28C26",
  width: 32,
  height: 32,
  borderRadius: 3,
...
const dayCell = {
  minHeight: 56,
  border: "1px solid rgba(242, 140, 38, 0.12)",
  borderRadius: 3,
  background: "transparent",
  display: "flex",
...
const lotRow = {
  borderBottom: "1px solid rgba(242, 140, 38, 0.08)",
  paddingBottom: 8,
};
...
const tbaToggle = {
  marginTop: 20,
  background: "transparent",
  border: "1px solid rgba(242, 140, 38, 0.18)",
  color: "#8A94A6",
  fontFamily: "monospace",
  fontSize: 13,
  fontWeight: 600,
